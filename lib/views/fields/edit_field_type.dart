...
class _EditFieldTypePageState extends State<EditFieldTypePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Field Type'),
      ),
      body: _isProcessing
          ? const Center(
        child: Column(
        ...
        child: Column(
          children: [
            ...
            if (newFieldType == 'booleanValue')
              DropdownButtonFormField<String>(
                initialValue: newFieldValue.isEmpty ? null : newFieldValue,
                onChanged: (value) {
                  setState(() {
                    newFieldValue = value!;
                  });
                ...
                items: ['true', 'false'].map((boolValue) {
                  return DropdownMenuItem<String>(
                    value: boolValue,
                    child: Text(boolValue, style: const TextStyle(
                      fontSize: 14.0,
                      fontWeight: FontWeight.normal,
                    ),),
                  );
